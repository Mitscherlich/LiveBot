> **Out of scope**: 实现阶段决策将此 capability 排除在本次变更之外。
> 当前项目使用 SQLite 短期记忆（`memory/short_term.py`）管理对话历史，
> `LLMPromptManager` / `HistoryResetPolicy` 的引入已推迟到后续独立变更。

## ADDED Requirements

<!-- 以下 requirement 已推迟，不在本次变更实现范围内 -->
