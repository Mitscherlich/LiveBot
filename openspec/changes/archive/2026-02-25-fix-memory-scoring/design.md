...
### 决策 5：多维评分公式替换单一 LLM 打分

**决策**：将 `_score()` 的单次 LLM 打分（0-10）替换为以下加权公式，阈值改为 > 0.5：

```
score = 0.3 × history_fullness    # 本地计算：min(recent_turns / promote_threshold, 1.0)
      + 0.2 × sentiment_intensity  # 本地计算：基于 user_emotion 标签
      + 0.2 × content_filter       # 本地计算：正则/关键词匹配个人信息/重要事件
      + 0.1 × memory_value         # LLM 打分归一化：_score() / 10
```

**本地分量定义：**
- `history_fullness`：近期对话轮数 / promote_threshold，越接近满窗越重要
- `sentiment_intensity`：`惊讶/悲伤/愤怒` → 1.0，`开心` → 0.5，`平静` → 0.2
- `content_filter`：对话文本包含姓名/偏好/约定/日期等关键词 → 1.0，否则 → 0.0

**LLM 调用优化：**
- 先计算本地分量之和（max 0.7）
- 若 `local_score > 0.5`，直接通过，**跳过** LLM 调用（节省 token）
- 若 `local_score + 0.1 <= 0.5`，直接不通过，**跳过** LLM 调用
- 只有 `0.4 < local_score <= 0.5` 时才调用 LLM 做最终判决

**理由**：大幅减少 LLM 评分调用（估计 70%+ 场景可跳过），评分更稳定可预测，同时保留 LLM 作为边界情况的最终仲裁。

**备选方案**：仅修复现有 LLM 评分 bug，不改变公式。弃用原因：单次 LLM 打分受模型随机性影响大，0-10 分范围下阈值 ≥7 相当严格，与实际需要的"边界模糊判断"不匹配。

---

### 决策 6：注入历史机制（injected_history）

**决策**：在 `CharacterConfig` 中新增 `injected_history: list[dict] = []` 字段，内容为固定的示范对话（user/assistant 交替，必须偶数条）。在 `pipeline/llm/base.py` 的消息构建阶段注入：

```python
messages = [
    {"role": "system", "content": system_prompt},
    *[{"role": m["role"], "content": m["content"]} for m in character.injected_history],
    *recent_history,          # 动态短期记忆（SQLite 最近 N 条）
    {"role": "user", "content": user_input},
]
```

**理由**：注入历史是引导 AI 风格最直接的方式，优先级高于短期记忆条目但低于系统提示词。通过 `character/config.yaml` 配置，不改变代码逻辑即可调整风格，对 Anthropic 实现同样适用（两者均继承 `base.py` 的消息构建）。

**格式约束**：
- 必须偶数条，user 在前 assistant 在后
- 若 `injected_history` 为空列表（默认），行为与现有完全一致

**备选方案**：将示范对话写入 system prompt 文本中。弃用原因：混合在 system 内，格式控制困难，Anthropic 等模型要求 messages 数组中 system 与对话严格分离。

---

## Open Questions

- 是否需要同步修复 `anthropic_llm.py`（若存在类似结构）？当前发现目录中有该文件，需确认是否也调用了 `maybe_promote`。确认：`base.py` 统一处理消息构建和 `maybe_promote`，`anthropic_llm.py` 继承后无需单独修改。
- 强制写入时的评分是否应记录为固定值（如 10）以便后续统计？建议记录 `score=1.0`（满分）以标识来源为强制写入。
